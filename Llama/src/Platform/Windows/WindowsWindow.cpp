...
#include "Llama/Events/KeyEvent.h"
#include "Llama/Events/MouseEvent.h"
...
namespace Llama
{
	...
	{
		LLAMA_CORE_ERROR("GLFW Error ({0}): {1}", error, description);//For now we just log it
	}
	...
	void WindowsWindow::Init(const WindowProps& props)
	{
		...
		LLAMA_CORE_ASSERT(status, "Failed to initialize Glad!");
		...
		glfwSetCharCallback(m_Window, [](GLFWwindow* window, unsigned int keycode)
			{
				WindowData& data = *(WindowData*)glfwGetWindowUserPointer(window);//We get Window data
